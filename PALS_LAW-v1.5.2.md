...
> **Coverage note:** The empirical references above directly support four of nine
> error classes: `ERR_HALLUCINATION` (Ji, Maynez, Lin), `ERR_SYCOPHANCY` (Perez,
> Sharma), `ERR_CALIBRATION` (Kadavath), and `ERR_REASONING` (Berglund). The
> remaining five classes (`ERR_OMISSION`, `ERR_SCHEMA`, `ERR_TRUNCATION`,
> `ERR_INSTRUCTION`, `ERR_SEMANTIC`) are defined in the taxonomy (§5) and are
