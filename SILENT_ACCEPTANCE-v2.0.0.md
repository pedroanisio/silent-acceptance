...
Asymmetry** (§7): as model capability grows, the errors that remain migrate toward the
...
- $\varepsilon(y, x) \in \{0, 1\}$ be a Boolean error predicate:
  ...
  enumerated in §5. Where a per-class predicate is needed, $\varepsilon_c(y, x)$
  denotes deviation in class $c \in \mathcal{C}$ alone, so that
  $\varepsilon = \max_{c \in \mathcal{C}} \varepsilon_c$.
- $V_c : \mathcal{Y} \times \mathcal{X} \to \{0, 1\}$ be a *verifier* for class $c$:
  ...
  $\varepsilon_c(y, x) = 1$. A verifier is fallible; its quality is its recall
...
- $R_c(V_c, M, \mathcal{D}) = P\bigl(V_c(y, x) = 1 \mid \varepsilon_c(y, x) = 1\bigr)$,
  with $x \sim \mathcal{D}$ and $y \sim P_\theta(\cdot \mid x)$, be the *recall* of
...
> for quantifying $\delta$ in that context. See §8.2.
...
> deployable risk model. See §8.3 for the full treatment.
...
> **Relationship to this specification:** These results independently establish what
> the specification takes as given: that LLM error is not an engineering defect to be
> debugged but a structural property of the model class. The contribution here is not
> the theoretical result but its packaging as a named, testable, enforceable
...
models ordered by $C$; the hypothesis predicts the sign of those differences per class.
...
4. **Estimate and test.** For each model, compute $C(M)$ and $D_c(M)$ for every
   class. Test monotonicity of $D_c$ in $C$ per class (for example, a rank
   correlation across the model sequence, with sign predicted by §7.1).
...
This is the basis of Corollary 5 (§9.6): treating the verifier as a stable component
while upgrading the model is an architectural regression, and a verifier upgrade is
a precondition for a model upgrade.
...
### 8.2 $\delta$ is task-dependent, model-dependent, and distribution-dependent
...
### 8.3 Independence assumption in pipeline compounding is approximate
...
1. **Scope** — which error classes $S \subseteq \mathcal{C}$ are checked, and, for
   ...
   mitigation note or a downstream boundary that covers it).
2. **Mechanism** — what each verifier $V_c$ is, so that its recall can in principle
   be measured (§8.4).
3. **Calibration** — the model version the boundary was calibrated against, because
   the boundary's adequacy is a function of $M$ (§7).
4. **Location** — where the verifier runs, and that the model, and any agent the
   model controls, cannot modify it (§9.7).
...
**Practical implication:** A system must not relax its verification boundary after
...
### 9.6 Corollary 5 — A verifier upgrade is a precondition for a model upgrade

If the Capability-Detection Asymmetry (§7) holds, a boundary calibrated against
model $M_1$ has lower recall on the semantic classes when applied to a more
capable $M_2$, precisely where the residual errors of $M_2$ concentrate.
...
model in any system where the prior boundary was calibrated to its predecessor.
Treating the verifier as a stable component while upgrading the model is an
architectural regression. This is why the contract block (§10.1) pins
`MODEL_VERSION`.

### 9.7 Corollary 6 — The verifier must sit outside the boundary it verifies
...
**Practical implication:** The verifier must be immutable from the model's
perspective — a separate process or privilege domain, versioned and reviewed like
production code, not writable by the agent whose output it judges — and its
verdicts must be recorded where a party outside the runtime can check them. This is
requirement 4 of §9.1. A "verification" step the agent can rewrite is not a boundary;
it is silent acceptance with extra steps.
 ...
 *   ↑ REQUIRED. §7 (Capability-Detection Asymmetry) establishes that
 *   verification requirements shift when model capability changes. This
 *   boundary is valid only for the pinned model. A model swap without
 *   boundary review is an architectural regression (§9.6).
 *
 ...
 *
 * For pipelines of length n with per-step error probabilities p_i ≥ δ > 0:
 *
 *   P(pipeline error-free) = ∏(1 - p_i) < 1
 *   P(at least one error)  → 1  as  n → ∞
 *   Note: assumes independent calls — see SILENT_ACCEPTANCE.md §8.3 for the
 *   correlation caveat.
 *
 ...
 *
 * MITIGATION: <for every unchecked class, either the downstream boundary that
 *   covers it or an accepted-risk note>
 *
...
```typescript
// ⚠ VERIFICATION BOUNDARY: LLM output is untrusted by default. Verify before use. (Silent Acceptance v2.0.0)
```
...
- a declaration with no `MODEL_VERSION:` pin.
...
artifacts, and the placement of this section respond to a review produced in
